...
prompt_completeness = """instruction: I will give you the response of a system to the user's utterance also I will give you some background information about the user and history of the user's conversation with the system. 
...
Please only generate an int score between -1 to 3 to say to what extent the response is a complete response to the user question.
"""
...
output_pkl = 'completeness-gpt4.pkl'
...
for index, row in df.iterrows():
    ...
    prompt_tmp = prompt_completeness.format( bkg=ptkb_turn[turn_id] , ctx=context_turn[turn_id] ,question=utterance_turn[turn_id], response= response)
...
with open('output/completeness_judgement-gpt4.txt', 'w') as f:
    f.writelines(lines)